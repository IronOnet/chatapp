...
// format the CreatedAt to display nicely on screen 
func (t *Thread) CreatedAtDate() string{
	return t.CreatedAt.Format("Oct 1, 2022 at 6:20pm")
}

func (p *Post) CreatedAtDate() string{
	return p.CreatedAt.Format("Oct 1, 2022 at 6:21pm")
}
...
func (user *User) CreateThread(topic string) (conv Thread, err error){
	...
	defer stmt.Close()
	// use QueryRow to return a row and scan the returned id into the 
	// session struct 
...
func (user *User) CreatePost(conv Thread, body string) (post Post, err error){
	statement := "inserte into posts (uuid, body, user_id, created_at) values ($1, $2, $3, $4) returning id, uuid, body, user_id, created_at"
	stmt , err := Db.Prepare(statement) 
	if err != nil{
		return 
	}
...
// Get the user who started a thread 
